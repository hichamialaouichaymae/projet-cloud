...
def kill_instance() -> dict:
    """Termine (terminate) une instance EC2 running au hasard — elle sera recréée par la recovery."""
    ec2 = _ec2()
    running = _running_instances(ec2)
    if not running:
        msg = "⚠️ Aucune instance running à terminer"
        log_event(msg)
    ...
    try:
        ...
        msg = f"💥 [CHAOS] Instance {iid} terminée (kill_instance)"
        log_event(msg)
