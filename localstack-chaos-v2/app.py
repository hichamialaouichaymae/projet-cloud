...
# FIX : chemin absolu pour le log (même référence que chaos_recovery.py)
...
def api_start(instance_id):
    try:
        ...
        log_event(f"▶️ Instance {instance_id} démarrée", LOG_PATH)
        return jsonify({"status": "success", "message": f"Instance {instance_id} démarrée"})
    except Exception as e:
        log_event(f"❌ start {instance_id} : {e}", LOG_PATH)
        return jsonify({"status": "error", "message": str(e)}), 500
...
def api_stop(instance_id):
    try:
        ...
        log_event(f"⏹️ Instance {instance_id} arrêtée", LOG_PATH)
        return jsonify({"status": "success", "message": f"Instance {instance_id} arrêtée"})
    except Exception as e:
        log_event(f"❌ stop {instance_id} : {e}", LOG_PATH)
        return jsonify({"status": "error", "message": str(e)}), 500
...
def api_terminate(instance_id):
    try:
        ...
        log_event(f"💥 Instance {instance_id} terminée", LOG_PATH)
...
def api_create_bucket(name):
    try:
        ...
        log_event(f"🔄 Bucket {name} créé", LOG_PATH)
...
if __name__ == "__main__":
    ...
    # FIX : utiliser sys.executable et chemin absolu pour les subprocesses
    app_dir = BASE_DIR
    subprocess.Popen([sys.executable, os.path.join(app_dir, "deploy.py")])
    subprocess.Popen([sys.executable, os.path.join(app_dir, "chaos_recovery.py")])
    subprocess.Popen([sys.executable, os.path.join(app_dir, "monitoring.py")])
